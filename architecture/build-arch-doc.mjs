// Build the client-facing platform architecture doc into .txt, .html, .docx and .pdf from the .md source.
// Offline + zero-install: pure Node for md->html/txt, the vendored `docx` lib for md->docx (Word),
// headless Chrome/Edge for html->pdf.
//   Usage:  node docs/architecture/build-arch-doc.mjs
// The .md is hand-authored; the other formats are always generated (never hand-edit them).
// (html/txt converter mirrors docs/build-docs.mjs; kept separate so this client deliverable is independent.)
...
// Optional argv override so a second doc in this folder can reuse the same pipeline:
//   node docs/architecture/build-arch-doc.mjs <base-filename> "<Title>"
// With no arguments the behaviour is unchanged (the platform architecture doc).
const BASE = process.argv[2] || "TrustBank-CBS-Platform-Architecture";
const TITLE = process.argv[3] || "TrustBank CBS — Platform Architecture";
...
// Inline: escape first, then code spans, bold, italics, then links (our content has no nested cases).
const inline = (s) =>
    escapeHtml(s)
...
// ---------- markdown -> html (subset: headings, hr, fences, tables, lists, quotes, paragraphs) ----------
...
const CSS = `
  ...
  h1 { font-size:26px; border-bottom:3px solid #0b5394; padding-bottom:8px; color:#0b5394; }
  h2 { font-size:19px; margin-top:30px; border-bottom:1px solid #e5e7eb; padding-bottom:4px; color:#0b5394; }
  ...
  th { background:#eaf2fb; color:#0b5394; }
  tr:nth-child(even) td { background:#fafafa; }
  blockquote { border-left:4px solid #0b5394; background:#eef5fc; margin:14px 0; padding:8px 14px; border-radius:0 6px 6px 0; color:#374151; }
...
// ---------- markdown -> docx (Word) via the repo-vendored `docx` library (offline, no install) ----------
// The `docx` package is vendored under docs/migration-estimate/node_modules (used by build_docx.js there).
try {
    ...
    const NAVY = "0B5394", CODEBG = "0F172A", CODEFG = "E2E8F0", HEADBG = "EAF2FB", ZEBRA = "FAFAFA";
    const thin = { style: BorderStyle.SINGLE, size: 1, color: "D1D5DB" };
    const cellBorders = { top: thin, bottom: thin, left: thin, right: thin };

    // inline markdown -> TextRun[] (handles **bold**, *italic*, `code`, [text](url))
    ...
    function mdToDocx(src) {
        ...
        while (i < lines.length) {
            ...
            // fenced code (e.g. the ASCII diagram) -> shaded monospace block, one paragraph per line
            if (/^```/.test(line)) {
                i++;
                const buf = [];
                while (i < lines.length && !/^```/.test(lines[i])) { buf.push(lines[i]); i++; }
                i++;
                buf.forEach((b, k) => out.push(new Paragraph({
            ...
            if (line.trim().startsWith("|") && i + 1 < lines.length && isTableSep(lines[i + 1])) {
                ...
                const cols = head.length, pct = Math.floor(100 / cols);
            ...
            // unordered list -> bullet paragraphs
            if (/^[-*]\s+/.test(line)) {
            ...
            // ordered list -> literal "n. text" paragraphs (robust across multiple lists)
            if (/^\d+\.\s+/.test(line)) {
    ...
    const doc = new Document({
        ...
        sections: [{
            ...
            footers: { default: new Footer({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: TITLE + "    ", size: 16, color: "888888" }), new TextRun({ children: ["Page ", PageNumber.CURRENT], size: 16, color: "888888" })] })] }) },
...
const profile = join(tmpdir(), "cbs-arch-doc-chrome");
